...
$ano = $_POST['ano'];
$data_ini = $ano."-".$_POST['mes_ini']."-01"; $data_fim = $ano."-".$_POST['mes_fim']."-31"; }
...
/*$conexao = mysql_connect('localhost','root','mysql') or die('Conexao falhou!');
...
for ($i = $_POST['mes_ini']; $i <=$_POST['mes_fim'];$i++){
$data_ini= $ano."-".$i."-01";
$data_fim= $ano."-".($i+1)."-01";
switch ($i) {
case 01:
...
//cria a query
$query = 'SELECT COUNT(*) as quantidade FROM ocorrencias o WHERE o.data_atendimento >= "'.$data_ini.'" AND o.data_atendimento < "'.$data_fim.'"' ;
//executa a query
...
echo '</td>';
} $query2 = 'SELECT COUNT(*) as em_tempo FROM ocorrencias o, sla_solucao sla, problemas p WHERE o.data_atendimento >= "'.$data_ini.'" AND o.data_atendimento < "'.$data_fim.'" AND o.data_fechamento AND p.prob_id = o.problema AND p.prob_sla = sla.slas_cod AND TIMESTAMPDIFF(MINUTE,o.data_abertura,o.data_fechamento) < sla.slas_tempo';
...
$query3 = 'SELECT COUNT(*) as fora_tempo FROM ocorrencias o, sla_solucao sla, problemas p WHERE o.data_atendimento >= "'.$data_ini.'" AND o.data_atendimento < "'.$data_fim.'" AND o.data_fechamento AND p.prob_id = o.problema AND p.prob_sla = sla.slas_cod AND TIMESTAMPDIFF(MINUTE,o.data_abertura,o.data_fechamento) > sla.slas_tempo';
...
$query4 = 'SELECT AVG(TIMESTAMPDIFF(HOUR,o.data_abertura,o.data_fechamento)) as media FROM ocorrencias o, sla_solucao sla, problemas p WHERE o.data_atendimento >= "'.$data_ini.'" AND o.data_atendimento < "'.$data_fim.'" AND o.data_fechamento AND p.prob_id = o.problema AND p.prob_sla = sla.slas_cod';
