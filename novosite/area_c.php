...
<?php 
  for ($i = 1; $i <=$ultimo_dia; $i++){
	...
	//cria a query
	$query = 'SELECT COUNT(*) as quantidade	FROM ocorrencias o, sistemas s, problemas p 
	WHERE o.problema = p.prob_id AND  p.prob_area = s.sis_id AND o.data_atendimento >= "'.$data_ini.'" AND p.prob_area = '.$area.' AND o.data_atendimento < "'.$data_fim.'"' ;
	//executa a query
