...
<?
	...
	if ((empty($action)) and empty($submit)){
        ...
        print "<TD align='right'><a href='predios.php?action=incluir'>Incluir prédio.</a></TD><BR>";
        if (mysql_numrows($resultado) == 0)
        {
                echo mensagem("Não existem prédios cadastrados no sistema!");
        }
        else
        {
                $cor=TAB_COLOR;
                ...
                print "<TD>";
                print "Existe(m) <b>$linhas</b> prédio(s) cadastrado(s) no sistema.<br>";
                print "<TABLE border='0' cellpadding='5' cellspacing='0'  width='50%'>";
                print "<TR class='header'><TD>Prédio</TD><TD><b>Alterar</b></TD><TD><b>Excluir</b></TD>";
                $j=2;
                while ($row=mysql_fetch_array($resultado))
                {
                        ...
                        print "<td><a onClick=\"confirma('Tem Certeza que deseja excluir esse prédio do sistema?','predios.php?action=excluir&cod=".$row['pred_cod']."')\"><img height='16' width='16' src='".ICONS_PATH."drop.png' title='Excluir o registro'></a></TD>";
                        print "</TR>";
				}
                print "</TABLE>";
        }
	...
	if (($action == "incluir")&& empty($submit)){
		...
		print "<B>Cadastro de Prédios:<br><a href='predios.php'>Listagem Geral</a>  <br>";
		print "<form name='incluir' action='".$PHP_SELF."' onSubmit='return valida()'>";
		print "<TABLE border='0' cellpadding='5' cellspacing='0' width='50%'>";
		print "<tr>";
		print "<td width='10%'bgcolor=".TAB_COLOR.">Prédio</td><td><input type='text' class='text' name='descricao' id='idDesc'></td>";
		print "</tr>";
	...
	if (($action=="alter") && empty($submit)){
		...
		print "<B>Alteração do nome do prédio:<br>";
		print "<form name='alter' action='".$PHP_SELF."' onSubmit='return valida()'>";
		print "<TABLE border='0' cellpadding='1' cellspacing='0' width='50%'>";
		print "<tr>";
		print "<td bgcolor=".TD_COLOR."><b>Prédio</b></td><td><input type='text' class='text' name='descricao' id='idDesc' value='".$rowAlter['pred_desc']."'>";
	...
	if ($action=="excluir"){
		...
		if ($linhas!=0) {
			print "<script>mensagem('Esse prédio não pode ser excluído pois existem departamentos associados a ele');
	...
	if ($submit=="Incluir"){
		if (!empty($descricao)){
			...
			if ($achou){
				...
				<script language="javascript">
				<!--
					mensagem('Esse Prédio já está cadastrado no sistema!');
					history.go(-2)();
				//-->
				</script>
				<?					
				...
				print "<script>mensagem('Dados incluídos com sucesso!');window.opener.location.reload(); redirect('predios.php');</script>";
				}
		} else {
