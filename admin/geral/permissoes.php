...
<?
	...
	if ((empty($action)) and empty($submit)){
        ...
        else
        {
                ...
                print "<TR class='header'><TD><b>Área</b></TD><TD><b>Módulo</b></TD><TD><b>Excluir</b></TD>";
                $j=2;
                while ($row=mysql_fetch_array($resultado))
                {
                        ...
                        print "<td><a onClick=\"confirma('Tem Certeza que deseja excluir essa permissão?','permissoes.php?action=excluir&cod=".$row['perm_cod']."')\"><img height='16' width='16' src='".ICONS_PATH."drop.png' title='Excluir o registro'></a></TD>";
                        print "</TR>";
				}
                print "</TABLE>";
        }
	...
	if (($action == "incluir")&& empty($submit)){
		...
		print "<TABLE border='0' cellpadding='5' cellspacing='0' width='50%'>";
		print "<tr><td>Área:</td><td><select class='select' name='area' id='idArea'>";
		...
		print "</select>";
		print "</td></tr>";
		print "<tr><td>Módulo:</td><td><select class='select' name='modulo' id='idModulo'>";
