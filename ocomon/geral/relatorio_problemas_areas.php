...
<?php
	...
	$s_page_ocomon = "relatorio_problemas_areas.php";
...
if ($ok != 'Pesquisar')   
{
	...
	print "	<BR><BR>";
	print "	<B><center>::: PROBLEMAS X ÁREAS DE ATENDIMENTO :::</center></B><BR><BR>";
	...
	print "							<option value=-1 selected>Relatorio</option>";
...
else //if $ok==Pesquisar
{
	...
	else
	{
		...
		if($d_ini_completa <= $d_fim_completa)	    { 
		       ...
		       echo "<br><br>";
			$background = '#CDE5FF';	 
			print "<p class='titulo'>CHAMADOS FECHADOS - PROBLEMAS X ÁREAS DE ATENDIMENTO</p>";
			print "<table class='centro' cellspacing='0' border='0' align='center'>";
			print "<tr><td colspan='2'><b>PERÍODO DE ".$d_ini." a ".$d_fim."</b></td></tr>";
			print "</table>";		   	   
		       echo "<br><br>";
		   
		   	$query .= " and o.data_fechamento >= '$d_ini_completa' and o.data_fechamento <= '$d_fim_completa'
                                 and o.data_atendimento is not null 
				GROUP BY p.problema, s.sistema 
                            ORDER BY quantidade DESC, p.problema";
			   ...
			   switch($saida)	{
				case -1: 
			   		$campos=array();	
						...
						while ($row = mysql_fetch_array($resultado)) {
							print "<tr>";
							print "<td>".$row['problema']."</td><td>".$row['area']."</td><td>".$row['quantidade']."</td>";
							print "</tr>";
				...
				case 1:
					...
					print "<img src='graph_problema_area.php?d_ini=".$d_ini."&d_fim=".$d_fim."'>";
					print "</div>";

					//Gera um gráfico pra cada área com os seus problemas
  					$query = 	"SELECT 
					                	s.sis_id,
								s.sistema  AS area,
								count(*)   AS quantidade
					...
					print "<p class='titulo'>ÁREAS DE ATENDIMENTO</p>";
  
  					while ($row = mysql_fetch_array($resultado)) {
						print "<div style='text-align:center'>";
						print "<img src='graph_areas.php?d_ini=".$d_ini."&d_fim=".$d_fim."&areaid=".$row['sis_id']."&area=".$row['area']."'>";
						print "</div>";
					}
