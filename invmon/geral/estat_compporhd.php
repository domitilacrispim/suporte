<?
  ...
  */
	...
	$s_page_invmon = "estat_compporhd.php";
...
######################################################################################################		
		...
		$query= "select count(*) as qtd, t.tipo_nome as equipamento, t.tipo_cod as tipo, h.mdit_cod as tipo_hd,
				concat(h.mdit_fabricante,' ',
				h.mdit_desc_capacidade,'GB') as hd, concat(count(*)/$total*100,'%') as porcento from 
...
######################################################################################
  ...
  while ($row = mysql_fetch_array($resultado)) {
				...
				<TD bgcolor=<?print $color;?>><a href=mostra_consulta_comp.php?comp_tipo_equip=<?print $row['tipo']?>&comp_modelohd=<?print $row['tipo_hd']?>&ordena=local,etiqueta title='Exibe a listagem de computadores cadastrados com esse modelo de HD.'><?print $row["hd"]?></a></TD>
