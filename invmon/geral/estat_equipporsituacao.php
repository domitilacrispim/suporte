<?
  ...
  */
	...
	$s_page_invmon = "estat_equipporsituacao.php";
...
######################################################################################
				  ...
				  if (($resultado = mysql_query($query)) && (mysql_num_rows($resultado) > 0) ) {
				  	while ($row = mysql_fetch_array($resultado)) {
						...
						?>
						<TR>
						<TD bgcolor=<?print $color;?>><a href=mostra_consulta_comp.php?comp_tipo_equip=<?echo $row['tipo_cod']?>&comp_situac=<?echo $row['situac_cod']?>&ordena=local,etiqueta title='Exibe a listagem dos equipamentos desse tipo.'><?print $row["equipamento"]?></a></TD>                
						<TD bgcolor=<?print $color;?>><a href=mostra_consulta_comp.php?comp_situac=<?echo $row['situac_cod']?>&ordena=local,etiqueta title='Exibe a listagem dos equipamentos cadastrados nessa situação.'><?print $row["situacao"]?></a></TD>
