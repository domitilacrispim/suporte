<?
  ...
  */
	...
	include ("../../includes/include_geral_II.inc.php");
		...
		$total = 0; $texto = "";
		$sql_1 = "SELECT * from equipamentos where comp_fornecedor='".$forn_cod."'";
		$exec_1 = mysql_query($sql_1);
		$total+=mysql_numrows($exec_1);
		if (mysql_numrows($exec_1)!=0) $texto.="equipamentos, ";
		
		$sql_2 = "SELECT * FROM softwares where soft_forn ='".$forn_cod."'";
		...
		if ($total!=0)
		{
			print "<script>mensagem('Este fornecedor não pode ser excluído, existem pendências nas tabelas: ".$texto." associados a ele!'); 
				redirect('fornecedores.php');</script>";
		}
		else
		{
			$query2 = "DELETE FROM fornecedores WHERE forn_cod='".$forn_cod."'";
