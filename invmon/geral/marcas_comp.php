<?
  ...
  */
	...
	$auth = new auth;
        ...
        print "<TR class='header'><TD><b>Código</TD><TD><b>Modelo</TD><TD><b>Tipo</TD><TD><b>Alterar</TD><TD><b>Excluir</TD>";
        $i=0;
        $j=2;
  		...
  		if (($resultado = mysql_query($query)) && (mysql_num_rows($resultado) > 0) ) {
			...
			if ($regImg!=0) {
                ...
                ?>
                <TD><a href='mostra_consulta_comp.php?comp_marca=<?print $row['codigo'];?>'><?print $row['codigo'];?></a></TD>
                <td><? print $linkImg."&nbsp;".$row['modelo'];?></td>
                <td><? print $row['tipo'];?></td>
                <?
				print "<td><a onClick=\"redirect('altera_dados_marca_comp.php?marc_cod=".$row['codigo']."')\"><img src='".ICONS_PATH."edit.png' title='Editar o registro'></a></TD>";
				print "<td><a onClick=\"confirma('Tem Certeza que deseja excluir esse registro do sistema?','exclui_dados_marca_comp.php?marc_cod=".$row['codigo']."')\"><img src='".ICONS_PATH."drop.png' title='Excluir o registro'></a></TD>";
				print "</TR>";
                $i++;
        	}
        }
